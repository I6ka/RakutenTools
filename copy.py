...
def create_pc_details():
    ...
    phone_code = create_phone_details(itemtitle,description_save,th_item_name,td_item_material,td_item_size)

    #ファイル名の入力と保存に遷移
    filesave(html_code_head,phone_code)
def create_pc_overray():
    ...
    '''.format(pc_overray=htmlfile_pc)

    html_code_head_phone = '''
    <html>
    <head>
        <meta charset='utf-8'>
    </head>
    ...
    '''.format(phone_overray=htmlfile_phone)

    filesave(html_code_head_pc,html_code_head_phone)
    
def create_phone_details(itemtitle,description_save,th_item_name,td_item_material,td_item_size):
    #ファイルの入力
    htmlfile = ""
    htmlfile += """
        ...
        """.format(itemtitle=itemtitle,description_save=description_save,th_item_name=th_item_name,td_item_material=td_item_material,td_item_size=td_item_size)
    ...
    '''.format(phone_overray=htmlfile)
    #ファイル名の入力と保存に遷移
    return html_code_head
...
#3_ファイル名を入力＆保存
def filesave(html_code,phone_code):
    filename_pc = input("保存するpc側のファイル名を入力してください：")
    filename_phone = input("保存するスマートフォン側のファイル名を入力してください：")
    path = os.path.dirname(__file__) + "/"
    file_pc = path + filename_pc + ".html"
    file_phone = path + filename_phone + ".html"
